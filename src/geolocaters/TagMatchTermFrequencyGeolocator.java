...
public class TagMatchTermFrequencyGeolocator extends AbstractGeolocator {
	...
	public TagMatchTermFrequencyGeolocator(String featurespath, String metapath,
	...
	private void AnalyseResults() {
		...
		System.out.println("Latitude error (in km) = " + Distance.distance(51.507222, -0.1275, 51.507222+latError/results.size(), -0.1275));
		
		System.out.println("Longitude error (in degrees) = " + lonError
				/ results.size());
		
		System.out.println("Longitude error (in km) = " + Distance.distance(51.507222, -0.1275, 51.507222, -0.1275+lonError/results.size()));
	...
	private void Test() {
		...
		while (!photos.isEmpty()) {
			...
			for (Integer tag : tags) {
				if (trainingSet.containsKey(tag) && trainingSet.get(tag).size()/totalImages < 0.5) {
